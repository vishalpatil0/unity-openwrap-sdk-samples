...
// Displays Video Interstitial ad and handles it's callbacks.
public class VideoInterstitial : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        interstitial.OnAdExpired += OnAdExpired;
    ...
    private void OnAdLoaded(object sender, EventArgs e)
    {
        ...
        Debug.Log("Video Interstitial : Ad Received");
    }

    // Callback method notifies an error encountered while loading or rendering an ad.
    private void OnAdFailedToLoad(object sender, POBErrorEventArgs error)
    {
        Debug.Log($"Video Interstitial : Ad failed to load with error - {error}");
    }
    ...
    {
        Debug.Log($"Video Interstitial : Ad failed to show with error - {error}");
    }
